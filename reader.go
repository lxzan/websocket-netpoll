...
func (c *Conn) readControl() error {
	...
	if n > 0 {
		if err := internal.CopyN(&buf, c.conn, int64(n)); err != nil {
			return err
		}
...
func (c *Conn) readMessage() error {
	...
	contentLength, err := c.fh.Parse(c.conn)
	if err != nil {
		return err
	}
	...
	if err := internal.CopyN(internal.Buffer{Buffer: buf}, c.conn, int64(contentLength)); err != nil {
		return err
	}
	if maskEnabled {
		internal.MaskXOR(buf.Bytes(), c.fh.GetMaskKey())
